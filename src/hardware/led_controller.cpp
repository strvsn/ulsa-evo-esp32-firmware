...
// UART Bridge is a double pulse, unlike the portal's continuous orange blink.
static constexpr uint32_t BRIDGE_PULSE_PERIOD_MS = 1600U;
static constexpr uint32_t BRIDGE_PULSE_ON_MS = 120U;
static constexpr uint32_t BRIDGE_SECOND_PULSE_START_MS = 300U;
...
void LedController::renderCurrentModeColor() {
  switch (_currentMode) {
    ...
    case MODE_STM32_UPDATE_TRANSFER:
      ...
      break;
    case MODE_COMMAND:
      setColor(255, 255, 255);  // 白: Command
      break;
    ...
    case MODE_INITIAL_CHECKING:
      ...
      break;
    case MODE_BRIDGE:
      setColor(255, 140, 0);  // 橙: UART Bridge
      break;
...
void LedController::update() {
  ...
  switch (_currentMode) {
    ...
    case MODE_BRIDGE: {
      const uint32_t elapsed = now - _lastBlinkTime;
      if (elapsed >= BRIDGE_PULSE_PERIOD_MS) {
        _lastBlinkTime += (elapsed / BRIDGE_PULSE_PERIOD_MS) * BRIDGE_PULSE_PERIOD_MS;
      }
      const uint32_t phase = now - _lastBlinkTime;
      const bool shouldBeOn = phase < BRIDGE_PULSE_ON_MS ||
          (phase >= BRIDGE_SECOND_PULSE_START_MS &&
           phase < BRIDGE_SECOND_PULSE_START_MS + BRIDGE_PULSE_ON_MS);
      if (_blinkState != shouldBeOn) {
        _blinkState = shouldBeOn;
        if (shouldBeOn) setColor(255, 140, 0); else setColorOff();
      }
      break;
    }
